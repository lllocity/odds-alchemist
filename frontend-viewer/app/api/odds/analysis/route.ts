...
verdict の定義と頭数上限:
- 本命: 1着を狙う最有力馬（軸）— 最大2頭まで
- 対抗: 三連複の軸2頭目、ワイドの相手筆頭（本命と組み合わせて三連複の軸を形成する）— 本命と対抗の合計が最大3頭になるように選出すること（本命1頭なら対抗は最大2頭、本命2頭なら対抗は最大1頭）
- 3着紐: 3着以内に絡む可能性がある馬（三連系の紐向き）— 最大5頭まで
- 消し: 馬券から外してよい馬（データ不足・推移不安定・支持なしすべて含む）

【重要】上限まで無理に埋める必要はない。根拠が薄い馬は積極的に「消し」にすること。
強い根拠がある馬だけを本命・対抗・3着紐に選出し、残りはすべて消しでよい。
本命と対抗の合計が3頭を超える場合は、超過分を3着紐に降格させること。本命は単独で最大2頭まで。
...
【出力前の必須チェックリスト - JSON生成前に以下を順番に実行すること】
① 本命の頭数を数える。3頭以上なら、根拠が最も薄い馬の verdict を「対抗」に変更し、comment も対抗候補としての評価に書き換えてから 2頭以下にすること。
② 本命＋対抗の合計を数える。4頭以上なら、根拠が最も薄い「対抗」の verdict を「3着紐」に変更し、comment も3着紐候補としての評価に書き換えてから合計 3頭以下にすること。
③ 3着紐の頭数を数える。6頭以上なら、根拠が最も薄い馬の verdict を「消し」に変更して 5頭以下にすること。
④ ①〜③をすべてパスした状態でのみ JSON を出力すること。
