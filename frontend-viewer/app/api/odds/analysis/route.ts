...
verdict の定義:
- 1着候補: 1着を狙う最有力馬（軸）
- 2着候補: 2着に来る可能性が高い馬（馬連・馬単の相手向き）
- 3着紐: 3着以内に絡む可能性がある馬（三連系の紐向き）
- 注目: データが不十分または推移が不安定で判断が難しい馬
- 消し: 馬券から外してよい馬

掛け合わせの目安:
- 低オッズ（15倍未満）＋強いトレンド → 1着候補の最有力
- 低オッズ（15倍未満）＋弱いトレンド → 2着候補 または 3着紐
- 中オッズ（15倍以上〜30倍未満）＋強いトレンド → 1着候補として十分あり
- 中オッズ（15倍以上〜30倍未満）＋弱いトレンド → 3着紐
- 高オッズ（30倍以上）＋強いトレンド・複数アラート → 2着候補 または 3着紐。1着候補とする場合はその根拠を comment に明記
...
7. アラートの活用（重要）
   ...
   - 複数のアラートが同一馬に集中している場合は1着候補として優先的に評価する
  ...
  "horses": [
    {
      "number": 馬番(数値),
      "name": "馬名",
      "verdict": "1着候補" または "2着候補" または "3着紐" または "注目" または "消し",
      "comment": "推移の観点から見たこの馬の評価と買い目における位置づけを2〜4文で。1着/2着/3着のどこに据えるべきか・外してよいかを明示すること",
