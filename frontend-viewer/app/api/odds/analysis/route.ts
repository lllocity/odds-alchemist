...
verdict の定義:
- 本命: 1着を狙う最有力馬（軸）
- 対抗: 本命に次ぐ総合的な勝ち負け候補。勝つ可能性もある2番手
- 2着候補: 勝ち切るより2着に来る可能性が高い馬（馬連・馬単の相手向き）
- 3着紐: 3着以内に絡む可能性がある馬（三連系の紐向き）
- 注目: データが不十分または推移が不安定で判断が難しい馬
- 消し: 馬券から外してよい馬

掛け合わせの目安:
- 低オッズ（15倍未満）＋強いトレンド → 本命・対抗の最有力
- 低オッズ（15倍未満）＋弱いトレンド → 2着候補 または 3着紐
- 中オッズ（15倍以上〜30倍未満）＋強いトレンド → 本命・対抗として十分あり
- 中オッズ（15倍以上〜30倍未満）＋弱いトレンド → 3着紐
- 高オッズ（30倍以上）＋強いトレンド・複数アラート → 2着候補 または 3着紐。本命とする場合はその根拠を comment に明記
- 高オッズ（30倍以上）＋弱いトレンド → 消し寄り
...
- 順位乖離: 単勝人気順位と複勝人気順位が大きくずれている → 穴馬として馬券に組み込む根拠
- トレンド逸脱: オッズ推移が通常パターンから外れた動き → 何らかの情報が先行している可能性
...
7. アラートの活用（重要）
   ...
   - アラートが発生した馬は verdict を "消し" にしてはならない
   - 支持率急増アラートはその馬を3着紐以上に押し上げる強いシグナルと解釈する
   - 複数のアラートが同一馬に集中している場合は本命・対抗候補として優先的に評価する
  ...
  "horses": [
    {
      "number": 馬番(数値),
      "name": "馬名",
      "verdict": "本命" または "対抗" または "2着候補" または "3着紐" または "注目" または "消し",
      "comment": "推移の観点から見たこの馬の評価と買い目における位置づけを2〜4文で。2着/3着に据えるべきか・外してよいか・軸とすべきかを明示すること",
