...
- `proxy.ts` は Node.js Runtime で動作するため、Next.js 15 時代に必要だった `auth.config.ts`（edge-compatible）と `auth.ts`（フル設定）の2ファイル分離は不要な可能性がある。ただし NextAuth v5 側の実装に依存するため、統合できるかどうかは実際に検証すること。現状の実装では2ファイル構成を採用している。
