...
- `node-html-parser` を依存追加。着順・レース名はテキスト解析でなく CSS クラス（`hr-denma__arrival`, `.hr-denma__race a`, `[class*="hr-label"]`）で取得
- `detail=1` ページは2テーブル構造: テーブル1（馬エントリ2列）→ 馬番の順番収集、テーブル2（詳細6列）→ 同インデックスで前走データを参照
- 距離別成績ページのレース距離は "芝・左 1600m" や "ダート・左 1600m" 形式（スペース入り）。正規表現は `[^\d]*` で空白を許容
- `achievement/distance` のテーブル列ヘッダー（"芝1600m", "ダ1600m" 等）から今走距離を特定。基本出馬表で取れなかった場合のフォールバックとして機能

**動作確認済み（安田記念 2026-06-08）:**
- 全17頭: 騎手名・前走レース名・前走着順・馬体重・芝1600m実績を正常取得
- 今走距離 `芝1600m` を achievement/distance ページから正常取得
...
- Step 38: ✅ 実装完了（2026-06-08）、動作確認済み
- Step 39: 未着手
