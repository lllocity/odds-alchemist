...
使用するモデルは分析前に選択可能。
各馬に「本命 / 対抗 / 3着紐 / 消し」の verdict と根拠コメントを付与し、
三連複・ワイドの推奨買い目を自動生成する。
...
verdict は「オッズの絶対水準 × トレンド強度」の掛け合わせで決定する（トレンド単独では判断しない）。

| オッズ水準 | トレンド強度 | 評価 |
|---|---|---|
| 低（15倍未満） | 強い | 本命の最有力 |
| 低（15倍未満） | 弱い | 対抗 または 3着紐 |
| 中（15〜30倍） | 強い | 本命として十分 |
| 中（15〜30倍） | 弱い | 3着紐 |
| 高（30倍超） | 強い ＋ 複数アラート | 対抗 または 3着紐 |
| 高（30倍超） | 弱い | 消し |

トレンド形状は4分類で判定する: 継続下落中 / 急落後安定 / 反発中（下落→上昇）/ 横ばい

### 複勝推移の活用（単勝との乖離を重視）

| 単勝推移 | 複勝推移 | 評価 |
|---|---|---|
| 下落 | 下落 | 本命の信頼度UP（市場全体が支持） |
| 横ばい / 高い | 下落 | 対抗を優先評価（「飛ぶよりは来る」） |
| 高い | のみ下落 | 3着紐（単勝人気は低いが3着以内は評価） |
| 上昇 | 上昇 | 消し（人気離れの複合シグナル） |

### 候補頭数の上限

| 区分 | 上限 |
|---|---|
| 本命 | 最大2頭（本命＋対抗の合計は最大3頭） |
| 対抗 | 本命＋対抗の合計が3頭以内に収まるよう調整 |
| 3着紐 | 最大5頭 |
| 消し | 制限なし |
