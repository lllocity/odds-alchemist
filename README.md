...
| メソッド | パス | 説明 | 利用FE |
|---|---|---|---|
| `GET` | `/api/odds/targets` | 登録済み監視URL一覧を取得 | 管理用 |
| `POST` | `/api/odds/targets` | 監視URLを追加 | 管理用 |
| `DELETE` | `/api/odds/targets` | 監視URLを削除 | 管理用 |
| `DELETE` | `/api/odds/sheets` | シートデータをクリア（`?sheet=OddsData\|Alerts`） | 管理用 |
| `GET` | `/api/odds/alerts` | 最新のアラート一覧を取得 | なし ※1 |
| `GET` | `/api/odds/history/urls` | OddsData に存在するURLの一覧を取得 | なし ※1 |
| `GET` | `/api/odds/history/horses` | 指定URL（`?url=`）の馬一覧を取得 | なし ※1 |
| `GET` | `/api/odds/history` | 指定URL・馬名（`?url=&horseName=`）のオッズ時系列データを取得 | なし ※1 |

> ※1 閲覧用 FE は Google Sheets を直接読み取る独自の Route Handler を持つため、バックエンドの閲覧系エンドポイントは現在いずれの FE からも呼ばれていない。
